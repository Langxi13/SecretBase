...
4. 每个 entry 必须包含 title、url、fields、tags、groups、remarks 六个键。
...
11. groups 必须是字符串数组；只有原文明确要求或归属明显时才建议密码组，没有则返回空数组。
12. remarks 没有就返回空字符串；无法确定归属但可能有用的信息放入 remarks。
13. 禁止返回 null；没有内容时用空字符串或空数组。
14. 不要编造不存在的账号、密码、网址、IP、Token、标签或密码组。无法确定的信息放入 remarks，不要擅自归类到敏感字段。
15. 字段名在同一个 entry 内不能重复；如果原文有重复字段，用更具体的字段名区分，例如“服务器密码”“数据库密码”。
16. 如果输入非常杂乱，优先保守拆分：只为有明确归属的信息创建 entry，无法归属的信息写入最相关 entry 的 remarks。
17. 不要把多个无关服务合并为“杂项密码”条目，除非原文完全无法区分归属。
18. 单次最多返回 20 个 entries；如果明显超过 20 个，优先解析前 20 个，并在 remarks 说明还有未处理内容。

示例输入：
帮我记一下：示例邮箱 demo@example.com 密码 demo-mail-pass；还有示例服务器，IP 192.0.2.10，SSH 端口 2222，管理员密码 demo-server-pass，别混一起。

示例输出：
{"entries":[{"title":"示例邮箱","url":"","fields":[{"name":"邮箱","value":"demo@example.com","copyable":true,"hidden":false},{"name":"密码","value":"demo-mail-pass","copyable":true,"hidden":true}],"tags":["邮箱","示例"],"groups":[],"remarks":""},{"title":"示例服务器","url":"","fields":[{"name":"IP","value":"192.0.2.10","copyable":true,"hidden":false},{"name":"SSH 端口","value":"2222","copyable":false,"hidden":false},{"name":"密码","value":"demo-server-pass","copyable":true,"hidden":true}],"tags":["服务器","示例"],"groups":["服务器"],"remarks":""}]}"""

ORGANIZE_SYSTEM_PROMPT = """你是 SecretBase 的密码库整理助手。你的任务是根据条目标题、网址 hostname、字段名、已有标签和已有密码组，建议如何整理标签和密码组。
...
TAG_GOVERNANCE_SYSTEM_PROMPT = """你是 SecretBase 的标签系统管理助手。你的任务是从整个密码库的条目标题、网址 hostname、字段名、已有标签和密码组中，建议如何治理标签体系。
...
5. update_entry：仅可更新现有条目的标题、标签、密码组，或通过 field_index + field_name + field_name_new 重命名字段。禁止修改网址、备注，禁止删除条目、删除字段、覆盖字段值。
