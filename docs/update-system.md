...
Android V5.0.0 至 V5.0.2 存在已知迁移问题：Manifest 缺少网络状态权限，默认开启自动预下载时可能在成功发现版本后显示检查失败。旧版用户可先关闭“自动预下载”再检查，或直接安装正式 V5.1+ APK。V5.1.1 起，网络状态读取、下载和安装界面启动失败都会保留手动重试入口；V5.1.2 进一步增加三次有限重试、HTTP Range 断点续传、部分文件保留和按设备 ABI 选择更新包；V5.1.3 修复重复后台指纹认证后可能残留软键盘 Insets 的界面问题。

Actions 候选包不是正式更新源。若仓库尚无包含签名清单的正式 Release，客户端会显示“暂无支持自动更新的正式版本”，不会把 GitHub 返回的 404 当作应用故障。正式 Release 创建后，三个客户端会自动读取其中的签名清单。
...
- Android 通用及 ABI 独立包、包名、`versionCode` 和正式证书 SHA-256 指纹。
...
Android 下载到应用缓存目录，网络中断后保留 `.part` 并使用受校验的 HTTP Range 继续；服务器忽略或拒绝续传时自动回退完整下载。下载结束后复核大小和 SHA-256，再由原生受限桥检查包名、版本号和签名。桥只接受缓存 `updates` 子目录中的 `.apk`，不能安装任意路径。没有“安装未知应用”权限时，应用只打开本包对应的系统授权页。

V5.1.2 的 Release 同时提供通用 APK 与三个 ABI 独立 APK。V5.1.1 及更早客户端只认识通用包，因此升级到 V5.1.2 时仍会下载一次通用 APK，并继续受旧下载器能力限制；V5.1.2 安装完成后，更新到后续版本时才会启用断点续传并优先选择约 21 至 26 MB 的设备匹配独立包。

V5.1.3 延续相同的包名、永久签名和 ABI 分发规则，可由 V5.1.2 直接覆盖更新。该版本只调整 Android 锁定页的输入焦点、软键盘关闭和窗口 Insets 恢复，不迁移 Vault 或应用私有数据。

V5.2.0 继续使用相同 Android 包名、永久签名和 ABI 分发规则，可从 V5.1.3 覆盖安装。Android 本轮不开放 WebDAV 同步界面，只更新共享 Rust Vault 依赖的 Sync Bundle V1 兼容能力；不会启动后台同步、迁移现有 Vault 或改变指纹与自动填充数据。

macOS 尚无 Developer ID 签名与公证，因此不自动替换 `/Applications`。手动覆盖 `.app` 不会删除 `~/Library/Application Support/SecretBase`。

macOS 构建和下载后复测对 DMG 创建、挂载与卸载执行最多三次有限重试，并保留 `hdiutil` 原始诊断输出；重试只作用于构建临时目录，不接触用户应用支持目录。
