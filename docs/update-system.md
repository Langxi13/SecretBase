...
Android V5.0.0 至 V5.0.2 存在已知迁移问题：Manifest 缺少网络状态权限，默认开启自动预下载时可能在成功发现版本后显示检查失败。旧版用户可先关闭“自动预下载”再检查，或直接安装正式 V5.1+ APK。V5.1.1 起，网络状态读取、下载和安装界面启动失败都会保留手动重试入口，并分别提示网络、DNS、VPN、系统时间或代理证书问题。
