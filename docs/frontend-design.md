...
备份管理 UI：独立备份中心在桌面端以两列展示手动备份和自动备份，移动端上下排列。两列各自拥有独立分页，每页 3 条；即使不足 3 条也保留占位槽并始终显示分页条，避免布局高度跳动。列表项展示文件大小、更新时间、条目数和回收站数，并提供指定备份下载和恢复按钮。操作按钮放在浅色圆角操作框中，并按动作使用低饱和浅色区分：加密下载偏蓝、明文 JSON 偏琥珀、恢复偏绿。恢复前必须进入三步向导并输入 `RESTORE`。
...
备份体验小修：手动备份创建期间显示 loading，成功后自动刷新备份列表并高亮新备份。备份中心必须说明手动备份不自动清理、自动备份会按保留数量轮转。恢复向导读取概况时必须显示正在读取提示，并在确认步骤强调会替换当前 vault。
