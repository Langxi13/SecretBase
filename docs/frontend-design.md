...
AI 管家分析范围：默认使用全部未删除条目。输入器通过清晰的范围按钮展示范围名称和条目数，点击后打开宽版弹窗，可切换全部条目、当前筛选结果和自定义选择。当前筛选结果覆盖所有分页；自定义选择支持标题或 hostname 搜索、标签、密码组、收藏筛选、跨页勾选、全选当前页以及 `5 / 10 / 20 / 50` 每页数量。选择列表只展示标题、hostname、标签和密码组，分页放在底部居中，避免依赖长列表滚动。切换会话、锁定或退出时清除自定义范围。

AI 管家计划审核：Web/桌面端允许标签、密码组、条目结构和条目创建组成一个原子复合计划，审核栏按领域分区并提供整组和逐项选择。普通操作默认勾选，标签删除/合并等高影响操作默认不选且显示风险标识；服务端返回的跨域冲突必须同时标注在总览和关联动作上，冲突项仍同时选中时禁用应用。一次应用只使用一个计划令牌、一个 Vault 快照和一个撤销入口，任何动作失败都不得部分保存。输入“确认”只应用当前可见计划，不再次调用模型。涉及现有条目的计划项提供紧凑的“查看条目”入口，在独立宽版详情层中按页切换关联条目并展示 URL、标签、密码组、全部字段和备注；隐藏字段默认遮罩，完整详情只从本地条目接口读取，不进入 AI 上下文。AI 请求处理中，专业工具和服务设置入口仍可打开，后台请求不得取消或重发。
...
搜索范围选择：搜索框下方必须提供可点击的范围框，包括标题、网址、标签、字段名、非隐藏字段值、备注。默认选中标题、网址、标签、字段名和备注，使搜索可以直接使用；非隐藏字段值必须由用户主动选择。全部取消时，输入搜索词不返回结果。
...
高级筛选增强：当前筛选条件必须以 chip 形式展示在筛选按钮下方，并支持单个条件快速移除。高级筛选新增有网址/无网址、有备注/无备注。常用筛选优先保存在浏览器本地 `localStorage`，不进入加密 vault；存储不可用时只影响跨会话记忆，不影响当前筛选和密码库操作。
...
桌面状态：仅在运行时 `mode=desktop` 时显示“桌面”设置页签，并在初始化页和锁定页提供状态入口。状态页展示版本、运行方式、WebView2、本地后端和固定数据目录检查；目录打开只允许数据、备份、日志三类。复制诊断摘要必须隐藏 Windows 用户名和绝对主目录。手动更新检查只在用户点击后执行，不在启动时联网。

关闭到托盘：桌面窗口允许缩小到 `360 × 320`，窄窗口沿用移动端布局和页面滚动。默认点击关闭按钮时显示应用内确认框，可选择隐藏到托盘、完全退出或取消；勾选“不再提醒”后保存本次选择，设置页的“关闭窗口时询问”可恢复提醒。保存设置只更新关闭偏好，不能立即创建托盘线程；托盘仅在真正隐藏时按需启动。进入托盘前必须锁定 vault、清除会话并通过桌面锁定事件立即清空前端敏感状态；最小化仍留在任务栏。托盘菜单只提供打开、锁定和退出，托盘不可用时窗口必须保持可见并提示失败。安装、升级和卸载使用独立退出信号，不能走普通关闭确认或隐藏到托盘分支。

锁定与写入反馈：桌面敏感内容保护层必须同时使用动画帧和定时器释放，并在前端初始化完成后清理残留状态，避免隐藏窗口暂停动画帧后遮住解锁页。确认框、修改主密码、条目、密码组、标签、标签合并和批量归组必须使用独立单次提交状态；处理中禁用确认、取消和关闭入口，按钮显示明确进度，锁定时统一清理可见弹窗和临时交互状态。

桌面平台能力：运行配置和诊断结果提供 `platform`、`architecture`、`renderer` 与 `capabilities`。前端只能根据能力字段显示托盘、目录入口和原生缩放提示，不得根据 UA 或平台文案猜测。macOS V3.3 首版隐藏托盘设置，关闭弹窗只提供退出和取消；Windows 保持现有托盘与缩放交互。

桌面导航与缩放：左侧栏只保留条目视图和常用标签，不重复放置顶部、底部已有的管理入口；短高度窗口使用紧凑间距保证“更多标签”和至少两个标签可见。桌面壳根据 `zoom_controls` 能力拦截 Windows `Ctrl + + / - / 0` 与 macOS `Command + + / - / 0`，浏览器模式不拦截。原生比例限制为 `25%-500%`，本机记忆最后比例并统一显示短暂百分比提示。
