...
AI 交互：用户可以输入自然语言指令，由后端生成结构化操作计划。前端必须提示不会发送字段值，先展示“操作计划”摘要和逐项勾选列表，再由用户点击“应用计划”执行。操作计划涉及现有条目时，后端必须补充 `entry_title` 或 `source_entry_title`，前端标题和条目行优先展示可读条目标题，不直接把唯一代码作为主要显示内容。第一版操作范围包括新建密码组、新建空条目、从现有字段复制生成新条目、更新条目的标题/网址/备注/标签/密码组/字段名；不提供删除条目、删除字段或覆盖字段值入口。字段拆分后原字段保留，真实字段值只由后端本地复制。
...
密码组模式：侧边栏“视图”区提供单一“密码组模式”入口，不在侧边栏平铺所有组。主内容区以卡片展示已有密码组，卡片包含名称、简介、条目数和最近更新时间；顶部主按钮必须是“新建密码组”，而不是“新建条目”。点击卡片后切回普通条目列表，并仅按该密码组筛选。进入具体密码组页后，列表顶部展示当前密码组操作条，提供“新建条目”和“选择条目”两个操作；新建条目会预填当前密码组，选择已有条目弹窗只展示尚未属于当前密码组的条目，支持按标签或密码组筛选、分页、当前页全选和批量加入，不提供搜索框。一个条目可属于多个密码组，编辑弹窗必须支持点选已有密码组，也允许输入新密码组和简介。
