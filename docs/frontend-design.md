...
条目列表提供搜索、标签筛选、密码组筛选、星标筛选、排序字段和排序方向控制。标签默认按覆盖条目数量降序展示，数量相同时按名称排序；“更多标签”弹窗提供条目数量升/降序和名称升/降序切换，名称排序使用浏览器中文排序规则，并使用分页展示标签，用户可通过下拉框选择每页 `5 / 10 / 20 / 50` 个标签，搜索、排序或每页数量变化后回到第一页，避免在弹窗内滚动长列表。回收站弹窗使用后端分页，避免超过第一页的删除项无法访问。
...
AI 助手：打开 AI 弹窗时调用 `GET /ai/status`。弹窗提供“智能录入”和“整理条目”两个模式。未配置时明确提示先进入设置页配置 Base URL、API Key 和模型，并提供“去配置 AI”；用户仍可选择“转为手动录入”把粘贴内容带入新建条目的备注。AI 解析失败时也必须保留原文并降级到手动录入。点击“智能解析”前必须确认 AI 已配置；解析中先显示正在解析状态，拿到解析结果后再进入冷却倒计时。AI 返回多条 `parsed_entries` 时，前端展示多条结果，并支持一次性创建多条新条目；单条结果继续进入新建条目表单供用户确认。AI 解析弹窗提供“清空解析”操作，用于清空输入、结果、失败提示和本地重复解析状态。

AI 整理条目：整理模式默认面向当前筛选全部条目，单次最多 100 条。前端必须提示“不会发送字段值”，只将标题、网址、字段名、已有标签和密码组等结构信息交给 AI。条目标签整理、密码组整理和标签系统管理必须分开执行，前端使用互斥模式切换，默认进入条目标签整理模式。AI 返回后先显示整体摘要，再以逐条建议形式展示新增/移除标签或密码组；标签系统管理展示新建、更新、删除、合并、替换、分配标签等动作。用户可取消某条建议或删除单个 chip，整体摘要必须随当前勾选和 chip 变更实时更新，点击“应用整理”后才写入。
...
标签管理：标签是独立实体，字段包含名称、简介和颜色，可以不绑定任何条目而单独存在。标签管理弹窗默认进入“标签列表”面板，不默认展开新建表单；点击“新建标签”或“编辑”后打开独立弹窗填写名称、简介和颜色。标签列表支持当前页选择、跨页保留选择、批量删除和分页管理，分页控件放在列表下方居中，用户可通过下拉框选择每页 `5 / 10 / 20 / 50` 个标签。合并标签作为独立面板展示，不和标签列表堆叠在同一个滚动区域，减少弹窗内长距离滑动。标签合并区域支持选择或输入源标签、输入目标标签后调用 `/tags/merge`，成功后刷新标签和条目列表。
