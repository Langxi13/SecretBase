...
当前设置页提供导出加密备份、导出明文 JSON、导入加密备份和导入明文 JSON。明文导入支持 `skip`、`overwrite`、`ask` 三种冲突策略。
...
备份管理 UI：设置页展示自动备份列表、文件大小、更新时间，并提供恢复按钮。恢复前必须二次确认。
...
备份体验增强：设置页必须提供手动创建备份入口。恢复备份前必须读取并展示备份概况，包括条目数、回收站条目数、文件大小和修改时间，并明确提示恢复会替换当前 vault。
...
备份体验小修：手动备份创建期间显示 loading，成功后自动刷新备份列表并高亮新备份。恢复前读取概况时必须显示正在读取提示，并在确认文案中强调会替换当前 vault。
