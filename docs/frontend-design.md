...
条目列表提供搜索、标签筛选、密码组筛选、星标筛选、排序字段和排序方向控制。标签默认按覆盖条目数量降序展示，数量相同时按名称排序；“更多标签”弹窗提供条目数量升/降序和名称升/降序切换，名称排序使用浏览器中文排序规则，并使用分页展示标签，用户可通过下拉框选择每页 `5 / 10 / 20 / 50` 个标签，搜索、排序或每页数量变化后回到第一页。选择 20 或 50 时，列表区域必须允许受控滚动访问全部标签，不能隐藏超出项。回收站弹窗使用后端分页，避免超过第一页的删除项无法访问。
