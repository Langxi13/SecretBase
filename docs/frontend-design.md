...
条目列表提供搜索、标签筛选、密码组筛选、星标筛选、排序字段和排序方向控制。标签默认按覆盖条目数量降序展示，数量相同时按名称排序；“更多标签”弹窗提供条目数量升/降序和名称升/降序切换，名称排序使用浏览器中文排序规则，并使用分页展示标签，搜索或排序变化后回到第一页，避免在弹窗内滚动长列表。回收站弹窗使用后端分页，避免超过第一页的删除项无法访问。
...
标签管理：标签是独立实体，字段包含名称、简介和颜色，可以不绑定任何条目而单独存在。标签管理弹窗默认进入“标签列表”面板，不默认展开新建表单；点击“新建标签”或“编辑”后打开独立弹窗填写名称、简介和颜色。标签列表支持当前页选择、跨页保留选择、批量删除和分页管理，分页控件放在列表下方居中。合并标签作为独立面板展示，不和标签列表堆叠在同一个滚动区域，减少弹窗内长距离滑动。标签合并区域支持选择或输入源标签、输入目标标签后调用 `/tags/merge`，成功后刷新标签和条目列表。
