...
AI 管家计划审核：标签、密码组和条目结构等不同管理领域必须拆成独立计划逐组确认；应用当前组后才能显示下一组，输入“确认”只应用当前可见计划，不再次调用模型。涉及现有条目的计划项提供紧凑的“查看条目”入口，在独立宽版详情层中按页切换关联条目并展示 URL、标签、密码组、全部字段和备注；隐藏字段默认遮罩，完整详情只从本地条目接口读取，不进入 AI 上下文。AI 请求处理中，专业工具和服务设置入口仍可打开，后台请求不得取消或重发。
