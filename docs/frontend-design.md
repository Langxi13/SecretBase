...
密码组模式：侧边栏“视图”区提供单一“密码组模式”入口，不在侧边栏平铺所有组。主内容区以卡片展示已有密码组，卡片包含名称、简介、条目数和最近更新时间；顶部主按钮必须是“新建密码组”，而不是“新建条目”。点击卡片后切回普通条目列表，并仅按该密码组筛选。进入具体密码组页后，列表顶部展示当前密码组操作条，提供“新建条目”和“选择条目”两个操作；新建条目会预填当前密码组，选择条目弹窗只展示尚未属于当前密码组的条目，支持搜索、全选可见和批量加入。一个条目可属于多个密码组，编辑弹窗必须支持点选已有密码组，也允许输入新密码组和简介。
