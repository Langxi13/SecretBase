...
AI 整理条目：整理模式默认面向当前筛选全部条目，单次最多 100 条。前端必须提示“不会发送字段值”，只将标题、网址、字段名、已有标签和密码组等结构信息交给 AI。整理标签和整理密码组默认都启用，可单独取消。AI 返回后先显示整体摘要，再以逐条建议形式展示新增/移除标签和密码组；用户可取消某条建议或删除单个 chip，点击“应用整理”后才写入。
