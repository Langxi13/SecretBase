...
- 模型响应若出现 `value`、`field_value`、`new_value`、`old_value`、`values`、未知引用或未知动作，必须在计划生成前拒绝。Web/桌面端可将标签、密码组、条目结构和条目创建动作放入一个复合计划，但动作正文仍只保存在服务端；应用前必须检测跨域命名、删除、分配和字段依赖冲突，并在单个 Vault 内存副本中全部成功后一次保存。任何失败都不得产生部分写入，整轮只创建一个加密快照和撤销令牌。
