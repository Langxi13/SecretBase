...
- 模型响应若出现 `value`、`field_value`、`new_value`、`old_value`、`values`、未知引用或未知动作，必须在计划生成前拒绝。混合标签、密码组或条目管理响应只能在服务端拆成独立待处理批次，每个批次使用新的计划令牌和 Vault revision 逐组确认，禁止一次应用多个管理域。
