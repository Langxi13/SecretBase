...
Vault V1 的规范性字节布局、payload 兼容规则和错误分类见 [SecretBase Vault V1 Format](vault-format-v1.md)。本文档中的示例代码只用于解释安全设计；若与格式规范或黄金向量不一致，以格式规范和黄金向量为准。
...
解密后的 V1 JSON 在根对象、条目和自定义字段层级保留未知字段，避免旧版本在保存无关修改时破坏未来扩展。生产实现仍拒绝 envelope 版本变化和非 V1 payload 主版本，不执行隐式迁移。
...
此处只描述本机 `secretbase.enc` 的 Vault V1 envelope。V5.2 的独立 Sync Bundle V1 使用固定 AAD 绑定对象类型、Vault UUID 和远端对象 ID，详见 [Sync Protocol V1](sync-protocol-v1.md)；两种二进制格式不得混用。
...
生产实现不记录请求体、条目标题、标签名、密码组名或字段内容；请求访问日志只保留路由模板、状态和耗时，业务操作日志只保留操作类型和数量。构建产物也不得携带本机日志目录。

### 7.4 对话式 AI 安全边界

AI 是可选的外部处理器，不属于 Vault 信任边界。Web、桌面和 Android 必须在本地构造受限 DTO，并在网络请求、模型响应归一化和本地执行三个层面同时执行以下规则：

- 普通管家模式只允许发送条目标题、网址 hostname、标签、密码组、字段名及隐藏/可复制状态；禁止发送已有字段 `value`、完整 URL、备注、主密码和真实条目 UUID。
- 每轮请求必须先调用不包含提示词的发送预检。用户确认前，提示词只能保留在当前页面；普通请求和 AI 新建都不得自动跳过逐次确认。
- 普通请求使用每轮重新生成的 `E001`、`E001.F01` 等临时引用。模型响应只能引用这些别名，服务端或 Rust 运行时在本地映射回真实条目和字段。
- 只有用户主动切换到“AI 新建”并二次确认后，才允许发送用户本轮输入的完整原文。敏感原文不写入对话历史，也不进入后续普通对话上下文。
- 普通输入检测到疑似密码、Token、API Key、私钥或高熵字符串时直接拒绝发送，并提示改用“AI 新建”。该检查同样覆盖旧的整理、标签治理和操作计划工具。
- 模型协议和执行器均不提供删除条目、删除字段、覆盖字段值、修改已有 URL/备注或删除密码组的动作。标签删除/合并必须预览确认；密码组只能由用户通过专用接口删除已确认为空的组。
- 模型响应若出现 `value`、`field_value`、`new_value`、`old_value`、`values`、未知引用或未知动作，必须在计划生成前拒绝。Web、桌面和 Android 均可将标签、密码组、条目结构和条目创建动作放入一个复合计划；应用前必须检测跨域命名、删除、分配和字段依赖冲突，并在单个 Vault 内存副本中全部成功后一次保存。任何失败都不得产生部分写入，整轮只创建一个加密快照和撤销令牌。
- 计划审核页展示的完整条目详情必须由客户端按需调用本地条目接口读取；模型响应只携带条目 ID 与标题作为本地定位信息，完整 URL、备注和字段值不得进入后续第三方 AI 请求。
- 所有写入计划保存在本机待处理区，前端只提交计划令牌和所选项目 ID，不能提交或修改动作正文。应用前再次校验解锁会话和 Vault revision。
- AI 写入前创建加密恢复快照。Web/桌面快照位于 `backups/ai`，最多保留 10 份或 7 天；Android 复用私有目录中的 10 份加密恢复副本。
- Web/桌面对话历史保存在 `ai-history.enc`，Android 保存在应用私有目录的 `ai-history.vault`。两者都使用与 Vault 主密钥用途隔离的派生密钥；修改主密码时同步重加密或安全清理。
- Base URL 默认必须使用 HTTPS，禁止凭据、查询参数和锚点；Web/桌面还会阻止受保护网络目标。API Key 只保存在用途隔离的加密设置文件中。
...
V5.2 同步不建立 SecretBase 官方账号或云端解密服务。用户选择自己的 WebDAV，客户端持有独立随机同步密钥，并只上传 Gzip 后的 AES-256-GCM 密文。

- 同步密钥与主密码、Vault 密钥和 WebDAV 密码相互独立。不同设备可以使用不同本地主密码加入同一同步 Vault。
- `head.sbh` 与全部 `.sbs` 快照都加密；WebDAV 操作者不能读取条目、字段、标签、密码组或设备显示名，但仍能观察目录 UUID、对象大小、数量、时间和请求来源。
- WebDAV 凭据和同步密钥保存在 `sync-settings.enc`，合并基线保存在 `sync-base.enc`。两者使用当前 Vault 密钥派生的独立用途密钥；修改主密码时原子重加密，导入不同 `vault_id` 时立即清除。
- 配对二维码、恢复码和配对 URI 等同于同步密钥，必须二次验证主密码后显示，不写入浏览器存储、日志、诊断和普通剪贴板历史。配对 URI 不包含 WebDAV 密码。
- 生产接口只允许 HTTPS 和系统信任的证书。不跟随重定向，不提供忽略 TLS 错误；本机 HTTP 仅由自动化测试注入的传输适配器使用。
- WebDAV 必须提供稳定强 ETag 并正确执行 `If-Match`、`If-None-Match`。客户端先写不可变快照，再条件提交加密 head，拒绝 last-write-wins 无条件覆盖。
- 同条目冲突只向界面暴露标题、状态、更新时间、字段数量和变化区块，不返回字段值。冲突计划绑定当前解锁会话、Vault revision 和远端 ETag。
- 最近 10 个加密历史版本意味着彻底删除的数据在旧快照淘汰前仍可恢复。用户需要更快清除时可连续产生新版本，或二次验证后删除整个远端同步空间。
- WebDAV `401/403` 映射为同步上游错误，不能触发 SecretBase 会话锁定。同步失败后的本机回滚必须继续遵守 revision 乐观锁，不能覆盖用户后续修改。

同步密钥轮换会用新密钥从当前文档重建历史，旧设备和旧恢复码立即失效。轮换期间若本机新配置无法持久化，实现会尽力用旧密钥恢复远端当前文档；无法恢复时必须给出受控错误，不能静默继续。
...
Android 客户端可选使用强生物识别解锁，但不保存主密码。用户开启功能时必须再次验证当前主密码，Rust 只导出包含 Vault salt 和当前派生密钥的设备凭据；该凭据立即由 Android Keystore 的认证绑定 AES-256-GCM 密钥加密，并保存在禁止系统备份的应用私有目录。

- 生物识别只允许 `BIOMETRIC_STRONG`，不接受系统 PIN、图案或锁屏密码代替。
- 设备支持时优先使用 StrongBox，不支持时回退到 Android Keystore。
- 指纹发生变化、Keystore 密钥失效、主密码修改、Vault 导入或恢复时删除旧凭据。
- 设备凭据等价于当前 Vault 解锁密钥，不得写入日志、备份、崩溃信息或跨设备导出文件。
- Dart、Kotlin 中可控的临时二进制缓冲区和 Rust 接收侧凭据在调用完成后主动清零；跨 FFI/平台通道的短暂复制不进入持久化，锁定与 Android 双击返回退出都会销毁 Rust 解锁会话。
- Vault V1 二进制格式不因生物识别改变，桌面与手机之间仍只通过加密 Vault 文件迁移。
...
| 无双因素认证 | 仅密码认证 | 可扩展支持 |
| WebDAV 流量元数据可见 | 服务端可观察密文大小、数量和访问时间 | 使用可信 WebDAV、HTTPS，并避免把目录地址公开 |
| 同步历史固定 10 版 | 彻底删除数据在旧加密快照淘汰前仍存在 | 删除远端同步空间或等待历史轮换 |
