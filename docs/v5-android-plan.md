...
- 移动 AI 输入区通过圆形 `+` 面板收纳快捷整理、模式、范围和专业工具；发送清单使用底部确认，计划按领域分组并折叠详情，标签删除与合并默认不选。
- AI 应用结果提供当前解锁会话内的一次性撤回；撤回必须匹配应用后的 revision，并恢复完整本机文档而不向模型发送字段值。
...
- 主密码不持久化；可选设备解锁密钥必须由强生物识别绑定的 Android Keystore 加密，锁定时清理解锁会话。
