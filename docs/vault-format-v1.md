...
- `vault_id`：可选的规范化 UUID。旧 Vault 可以缺省；首次启用同步时必须生成并持久化，新客户端创建 Vault 时应直接生成。
...
兼容实现不得仅因为模型增加了 `vault_id` 就给旧文档写入 `vault_id: null`。字段缺省时应继续保持缺省；一旦存在则必须是有效 UUID，并在保存时规范化为小写连字符形式。
