...
assertIncludes(aiStateJs, "const aiMode = ref('parse')", 'AI 弹窗必须有 parse/organize 模式状态');
assertIncludes(aiStateJs, "const aiActionInstruction = ref('')", 'AI 交互必须维护用户自然语言指令');
assertIncludes(aiStateJs, 'const aiActionResult = ref(null)', 'AI 交互必须维护操作计划结果');
assertIncludes(aiStateJs, 'const aiOrganizePrompts = reactive', 'AI 整理三个子功能必须维护独立本次偏好');
assertIncludes(aiStateJs, 'const currentAiOrganizePrompt = computed', '必须根据当前整理子模式读取偏好提示词');
assertIncludes(aiFeatureCompositionJs, 'window.SecretBaseAiController.createAiController', 'AI 领域装配模块必须装配专业工具控制器');
...
assertIncludes(viewHelpersJs, 'action.entry_title || action.source_entry_title', '更新条目计划标题必须优先展示条目标题');
assertIncludes(viewHelpersJs, "return action.source_entry_title || '来源条目'", '字段拆分计划缺少标题时必须使用中文兜底');
if (viewHelpersJs.includes('action.entry_title || action.entry_id') || viewHelpersJs.includes('action.source_entry_title || action.source_entry_id')) {
    throw new Error('AI 交互界面不得直接展示唯一条目代码');
}
...
assertIncludes(aiFeatureCompositionJs, 'window.SecretBaseAiView.createAiView', 'AI 领域装配模块必须复用 AI 视图摘要模块');
...
assertMatches(aiStateJs, /organizeTags:\s*true[\s\S]*organizeGroups:\s*false/, '默认只能整理标签，不能同时整理标签和密码组');
