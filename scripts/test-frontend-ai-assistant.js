...
const localActions = read('frontend/js/controllers/ai-assistant-local-actions.js');
const inspectorController = read('frontend/js/controllers/ai-assistant-inspector-controller.js');
const planHelpers = inspectorController;
...
const inspectorState = read('frontend/js/ai-assistant-inspector-state.js');
const sessionController = read('frontend/js/app-session-controller.js');
const sessionSecurity = read('frontend/js/app-session-security.js');
const aiWorkspaceCss = read('frontend/css/ai-workspace.css');
const aiInspectorCss = read('frontend/css/ai-entry-inspector.css');
const aiPlanCss = read('frontend/css/ai-plan-review.css');
...
assertIncludes(markup, 'type="button" class="ai-header-action" @click="openProfessionalAiTools"', '专业工具入口必须使用不提交表单的按钮');
assertIncludes(markup, 'type="button" class="ai-header-action" @click="openAssistantSettings"', '服务设置入口必须使用不提交表单的按钮');
assertIncludes(markup, '@click="openProfessionalAiTools" :disabled="aiAssistantBusy || !!aiAssistantPrepared"', '发送或确认期间必须锁定专业工具入口');
assertIncludes(markup, '@click="openAssistantSettings" :disabled="aiAssistantBusy || !!aiAssistantPrepared"', '发送或确认期间必须锁定服务设置入口');
assertIncludes(markup, '应用已选计划', 'AI 写入必须经过逐项计划审核');
assertIncludes(markup, 'v-for="group in aiAssistantPlan.actionGroups"', '复合计划必须按管理领域分组展示');
assertIncludes(markup, 'class="ai-plan-group-header"', '每个复合计划分组必须提供整组选择操作');
assertIncludes(markup, '需要处理的冲突', '复合计划必须可视化服务端冲突');
assertIncludes(markup, '撤销本次操作', 'AI 写入后必须提供恢复快照撤销入口');
assertIncludes(markup, '查看条目{{ action.entryTargets.length > 1', 'AI 建议必须提供关联条目详情入口');
assertIncludes(markup, 'class="ai-entry-inspector-overlay"', 'AI 建议条目必须在独立详情层展示，避免挤压计划栏');
assertIncludes(markup, '完整内容仅从当前本机密码库读取，不会补发给 AI', '建议详情必须明确完整内容只在本地读取');
assertIncludes(controller, '/ai/assistant/turns/preview', '前端必须先请求不含提示词的发送清单');
assertIncludes(localActions, 'window.crypto.getRandomValues', '本地密码生成必须使用浏览器加密随机数');
...
assertIncludes(planHelpers, "const confirmationMessages = new Set", '常见确认词必须由本地计划状态机处理');
assertIncludes(planHelpers, 'actionGroups: Array.from(groupsByDomain.values())', '混合任务必须归一化为一个分组复合计划');
assertIncludes(planHelpers, 'function assistantPlanHasSelectedConflicts', '选中冲突操作时必须阻止复合计划提交');
assertNotIncludes(controller, 'result.data?.next_plan', '复合计划不得继续生成逐组计划队列');
assertIncludes(inspectorController, 'const entry = await store.getEntry(normalizedId)', '建议完整条目必须按需从本地接口读取');
assertIncludes(composition, 'window.SecretBaseAiAssistantController.createAiAssistantController', 'AI 领域装配必须接入对话管家控制器');
assertIncludes(composition, 'openEntryDetail,', 'AI 领域装配必须接入本地条目详情打开动作');
...
assertMatches(
    controller,
    /finally\s*\{[\s\S]*aiAssistantBusy\.value\s*=\s*false;[\s\S]*aiAssistantStage\.value\s*=\s*'';/,
    '本地回复或请求失败后必须结束处理中状态'
);
assertIncludes(localActions, 'const limit = Math.floor(0x100000000 / alphabet.length)', '本地密码生成必须使用无取模偏差的拒绝采样');
...
assertMatches(
    aiInspectorCss,
    /\.ai-entry-inspector\s*\{[\s\S]*?width:\s*min\(1080px,\s*100%\)[\s\S]*?height:\s*min\(760px,/,
    '建议条目详情必须使用宽屏独立查看层'
);
assertMatches(
    aiPlanCss,
    /\.ai-plan-group\s*\{[\s\S]*?border-left:\s*3px\s+solid\s+var\(--plan-domain-color\)/,
    '复合计划分组必须使用清晰但克制的领域区分'
);
...
assertIncludes(inspectorState, 'const aiAssistantInspector = reactive({', 'AI 管家必须维护独立的建议条目详情状态');
assertIncludes(aiState, 'window.SecretBaseAiAssistantInspectorState.createAiAssistantInspectorState', 'AI 状态必须装配独立建议详情状态模块');
assertIncludes(aiState, "!window.matchMedia('(max-width: 820px)').matches", '桌面端对话历史必须默认展开，窄屏默认收起');
assertIncludes(sessionSecurity, "setRef('showAiAssistant', false)", '锁定密码库时必须关闭 AI 管家');
assertIncludes(controller, 'function disposeAiAssistant()', '锁定或卸载时必须使 AI 管家异步请求失效');
assertIncludes(controller, 'if (!isCurrentAssistantSession(epoch)) return', 'AI 管家迟到响应不得回写新会话');
assertIncludes(sessionSecurity, 'state.resetAiAssistantSession()', '锁定密码库时必须清除 AI 管家敏感状态');
