...
assertMatches(
    styleCss,
    /\.entry-card \.field-name,\s*\.entry-card \.field-value\s*\{[\s\S]*?overflow:\s*hidden[\s\S]*?text-overflow:\s*ellipsis[\s\S]*?white-space:\s*nowrap[\s\S]*?overflow-wrap:\s*normal/,
    '卡片内长字段名和值必须单行省略，不能挤压或覆盖相邻内容'
);
