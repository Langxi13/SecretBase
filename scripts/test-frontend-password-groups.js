...
assertIncludes(storeJs, 'async updateGroupOrder', 'Store 必须封装密码组自定义排序接口');
assertIncludes(storeJs, "api.post('/groups/order'", '密码组排序必须调用集合级排序接口');
...
assertIncludes(appJs, 'function openEditGroupModal', '必须提供编辑密码组弹窗入口');
assertIncludes(appJs, 'async function moveGroupOrder', '密码组卡片必须支持直接移动排序');
assertIncludes(appJs, 'async function resetGroupOrder', '密码组模式必须支持恢复默认条目数排序');
assertIncludes(appJs, 'function groupOrderNamesAfterMove', '前端必须基于当前卡片顺序计算新排序');
assertIncludes(appJs, 'function groupCardStyle', '密码组卡片必须提供颜色变量计算方法');
assertIncludes(appJs, 'function entryCardStyle', '条目卡片必须提供颜色变量计算方法');
assertIncludes(appJs, 'function visibleEntryGroups', '条目卡片必须限制密码组 chip 数量并提示剩余数量');
assertIncludes(appJs, 'async function saveGroup', '必须提供保存密码组方法');
assertIncludes(appJs, 'editingGroupName', '编辑密码组时必须记录原密码组名称');
assertIncludes(storeJs, 'async updateGroup', 'Store 必须封装密码组更新接口');
...
assertMatches(
    styleCss,
    /\.group-card-action-btn[\s\S]*?height:\s*18px[\s\S]*?font-size:\s*11px[\s\S]*?white-space:\s*nowrap/,
    '密码组卡片操作按钮必须进一步压缩到 18px 高度且不换行'
);
...
assertMatches(
    styleCss,
    /\.group-card-date::before[\s\S]*?right:\s*142px[\s\S]*?border-top:\s*1px dashed var\(--border-color\)/,
    '密码组日期虚线必须避开右侧操作区'
);

assertMatches(
    styleCss,
    /\.group-card[\s\S]*?--group-accent[\s\S]*?background:[\s\S]*?color-mix\(in srgb, var\(--group-accent\) 8%, var\(--bg-card\)\)/,
