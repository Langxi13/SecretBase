...
assertMatches(storeTaxonomyMethodsJs, /async loadGroups\s*\(/, 'Store 必须提供 loadGroups');
...
assertIncludes(entryControllerJs, "filter.value === 'group'", '统一新建入口必须识别当前密码组上下文');
assertIncludes(entryControllerJs, 'entryForm.groups = [activeGroupName.value]', '统一新建入口必须预填当前密码组');
...
assertNotIncludes(indexHtml, 'openCreateEntryForActiveGroup', '密码组新建条目必须统一走通用入口，避免空状态与已有条目状态行为不一致');
...
assertMatches(
    indexHtml,
    /<button[^>]*@click="showGroupMode"[^>]*>\s*返回密码组\s*<\/button>/,
    '具体密码组页必须提供明确返回入口'
);
assertMatches(
    indexHtml,
    /<button[^>]*@click="confirmDeleteGroup\(activeGroup\)"[^>]*>\s*删除密码组\s*<\/button>/,
    '具体密码组页必须提供删除入口'
);
...
const runtime = vm.createContext({ window: null });
runtime.window = runtime;
vm.runInContext(entryControllerJs, runtime, { filename: 'frontend/js/controllers/entry-controller.js' });

const filter = { value: 'group' };
const activeTagName = { value: '' };
const activeGroupName = { value: '工作账号' };
const entryForm = { tags: ['旧标签'], groups: ['旧密码组'] };
const showCreateModal = { value: false };
const entryActions = runtime.SecretBaseEntryController.createEntryController({
    filter,
    activeTagName,
    activeGroupName,
    entryForm,
...
entryActions.openCreateModal();
if (entryForm.groups.length !== 1 || entryForm.groups[0] !== '工作账号' || !showCreateModal.value) {
    throw new Error('密码组已有条目时，从统一新建入口打开编辑器仍必须默认选中当前密码组');
}
