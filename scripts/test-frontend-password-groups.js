...
assertIncludes(indexHtml, '?v=20260709-ui-v55', '前端资源版本必须随密码组排序按钮 UI 修复更新，避免浏览器继续使用旧 JS/CSS');
...
assertMatches(
    styleCss,
    /\.group-mode-actions[\s\S]*?flex-wrap:\s*nowrap/,
    '恢复默认排序和新建密码组必须保持同一行'
);

assertMatches(
    styleCss,
    /\.group-card-action-btn[\s\S]*?height:\s*22px[\s\S]*?font-size:\s*var\(--font-size-xs\)/,
    '密码组卡片操作按钮必须压缩到 22px 高度，避免压过虚线'
);
