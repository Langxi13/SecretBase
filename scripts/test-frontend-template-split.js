...
assertIncludes(indexHtml, `js/template-loader.js?v=${assetVersion}`, '入口页必须在 app.js 前加载模板加载器');
assertIncludes(loaderJs, 'runtimeConfig.assetVersion || runtimeConfig.version', '模板缓存版本必须复用独立资源修订号');
...
assertLessThan(indexHtml.split('\n').length, 105, 'index.html 必须保持为轻量入口页');
