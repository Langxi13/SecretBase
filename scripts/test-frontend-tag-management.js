...
assertMatches(
    indexHtml,
    /<div class="tag-manager-toolbar"[\s\S]*openCreateTagModal[\s\S]*<div v-(?:else-)?if="tags\.length === 0"/,
...
assertMatches(
    watchersJs,
    /watch\(state\.tagBrowserPageSize[\s\S]*normalizePageSize\(state\.tagBrowserPageSize, 'secretbase\.tagBrowserPageSize'/,
    '更多标签每页数量变化后必须写入本地偏好'
);
assertMatches(
    watchersJs,
    /watch\(state\.tagManagerPageSize[\s\S]*normalizePageSize\(state\.tagManagerPageSize, 'secretbase\.tagManagerPageSize'/,
    '标签管理每页数量变化后必须写入本地偏好'
);
