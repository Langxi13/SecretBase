...
const sessionLifecycleSource = read('frontend/js/app-session-lifecycle.js');
const sessionRuntimeSource = `${sessionSource}\n${sessionLifecycleSource}`;
...
assertIncludes(indexHtml, `js/controllers/desktop-controller.js?v=${assetVersion}`, '入口页必须加载桌面控制器');
assertIncludes(indexHtml, `css/desktop-components.css?v=${assetVersion}`, '入口页必须加载桌面样式');
...
assertIncludes(settingsTemplate, 'desktopUpdateAutoCheck', '桌面设置必须允许关闭自动检查');
assertIncludes(settingsTemplate, 'desktopUpdateAutoDownload', 'Windows 安装版必须允许关闭预下载');
assertIncludes(settingsTemplate, '@click="handleDesktopUpdateAction"', '桌面更新必须提供下载或安装动作');
...
assertIncludes(sessionRuntimeSource, "window.addEventListener('secretbase:desktop-lock'", '桌面壳锁定必须立即通知前端清空解锁态');
assertIncludes(sessionRuntimeSource, 'window.SECRETBASE_DESKTOP_LOCK_READY = true', '前端必须向桌面壳声明锁定事件已就绪');
assertIncludes(sessionRuntimeSource, "window.addEventListener('secretbase:desktop-close-request'", '桌面壳必须能请求前端显示关闭确认');
assertIncludes(sessionRuntimeSource, 'store.setState({ locked: true })', '桌面锁定必须同步更新 store 状态');
            ...
            api: {
                ...
                async check_for_updates() {
                    ...
                    return {
                        ...
                        manual_download_url: 'https://github.com/Langxi13/SecretBase/releases/download/v3.2.1/SecretBase-v3.2.1-windows-x64-setup.exe',
                        install_supported: true
                    };
                },
                async get_update_state() {
                    return { status: 'idle', preferences: { auto_check: true, auto_download: true } };
                },
                async start_background_update_check() {
                    return { status: 'idle', preferences: { auto_check: true, auto_download: true } };
                },
                async set_update_preferences(autoCheck, autoDownload) {
                    nativeCalls.push(['update-preferences', autoCheck, autoDownload]);
                    return { status: 'idle', preferences: { auto_check: autoCheck, auto_download: autoDownload } };
                },
...
(async () => {
    ...
    if (openedUrls.length !== 1 || !openedUrls[0].includes('/releases/download/v3.2.1/')) throw new Error('更新下载页未打开');
