...
| 导入导出 | 手动/自动备份分离、加密备份导出、明文 JSON 导出、导入预览、恢复、旧备份兼容。 |
...
- 应用内自动备份保存在 `BACKUP_DIR/auto/` 并按数量轮转；手动备份保存在 `BACKUP_DIR/manual/`，不会被自动轮转删除。
...
| Import and export | Separate manual and automatic backups, encrypted backup export, plain JSON export, import preview, restore, and legacy backup compatibility. |
...
- In-app automatic backups are stored in `BACKUP_DIR/auto/` and rotated by count; manual backups are stored in `BACKUP_DIR/manual/` and are not removed by automatic rotation.
