...
默认 AI 工作区使用对话式管家。普通模式只发送条目标题、网址 hostname、标签、密码组、字段名和字段属性，并使用 `E001` 等每轮临时别名；已有字段值、完整 URL、备注和真实条目 ID 不会进入网络请求。只有用户主动切换到“AI 新建”并二次确认后，才允许发送本轮输入的完整原文；该原文不会写入对话历史或后续普通上下文。
