...
class RustBuilder {
  ...
  String get _toolchain =>
      Environment.rustToolchain ?? _buildOptions?.toolchain.name ?? 'stable';
  ...
  Future<Map<String, String>> _buildEnvironment() async {
    final Map<String, String> buildEnvironment;
    if (target.android == null) {
      ...
      buildEnvironment = await env.buildEnvironment();
    }
    return _withRemappedBuildPaths(buildEnvironment);
  }

  Map<String, String> _withRemappedBuildPaths(
    Map<String, String> buildEnvironment,
  ) {
    final encodedFlags = <String>[
      if (buildEnvironment['CARGO_ENCODED_RUSTFLAGS'] case final flags?
          when flags.isNotEmpty)
        flags,
      '--remap-path-prefix=${_sourceRoot()}=/workspace/secretbase',
      if (_homeDirectory() case final home?)
        '--remap-path-prefix=$home=/build-home',
      if (environment.androidSdkPath case final sdkPath?)
        '--remap-path-prefix=$sdkPath=/android-sdk',
    ];
    return {
      ...buildEnvironment,
  ...
  String _sourceRoot() {
    var current = Directory(environment.manifestDir);
    while (current.parent.path != current.path) {
      final marker = path.join(current.path, '.git');
      if (Directory(marker).existsSync() || File(marker).existsSync()) {
        return path.canonicalize(current.path);
      }
      current = current.parent;
    }
    return path.canonicalize(
      path.join(environment.manifestDir, '..', '..', '..'),
    );
  }
  ...
  String? _homeDirectory() {
    ...
    return home == null || home.isEmpty ? null : path.canonicalize(home);
  }
}
