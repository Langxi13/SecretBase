...
- `v5.3.2` 是当前稳定版本；Android 在保持本地 Vault、自动填充、更新和生命周期保护的基础上，支持用户自有 HTTPS WebDAV 的 V2 加密快照同步，不依赖强 ETag，并支持扫描或粘贴配对链接加入同步空间。
- Android CI 同时生成通用 APK 与 `armeabi-v7a`、`arm64-v8a`、`x86_64` 独立 APK，并继续执行 API 29/36 模拟器启动验证。
...
- 跨设备同步支持创建/加入空间、连接编辑、自动同步开关、恢复码、弱网重试、无 ETag 上传校验、并发分支合并、脱敏冲突处理、历史查看/恢复、密钥轮换、显式历史压缩和远端删除；压缩后其他设备必须重新加入。
- 加密备份导入导出、恢复记录和写入 revision 冲突保护。
- 默认对话式 AI 管家，支持加密历史、全部/指定条目范围、圆形 `+` 快捷操作面板、AI 新建、条目定位、底部发送确认、跨标签/密码组/条目结构的复合计划、按领域折叠审核和 revision 绑定的一次性撤回；高影响标签操作默认不选，原五项专业工具继续保留并复用同一审核界面；请求处理中可取消，迟到响应不会覆盖新请求。
...
同步连接、独立同步密钥和本机同步基线使用 Vault 用途隔离密钥加密保存。恢复码、历史恢复、密钥轮换、压缩和远端删除要求再次验证主密码；锁定 Vault 会关闭活动 WebDAV 客户端并取消未完成请求。远端只保存 `SBS2` 加密快照，WebDAV 服务仍可观察对象数量、大小、UUID 和访问时间。
...
本机不要日常并行构建三个 ABI；正式通用包、三架构独立包与 API 29/36 模拟器交给 `.github/workflows/android.yml`。
