...
结构：{"entries":[{"title":"名称","url":"https://example.com","fields":[{"name":"字段名","value":"字段值","copyable":true,"hidden":false}],"tags":["标签"],"remarks":"备注"}]}
规则：entries 永远是数组，最多 20 条；不同服务必须拆成不同条目；每条必须包含 title、url、fields、tags、remarks；url 只能是 http/https，否则为空；字段名不能重复；密码、密钥、Token、API Key、恢复码、卡号等敏感字段 hidden=true，账号、邮箱通常 hidden=false；不要编造输入中不存在的值；没有内容用空字符串或空数组，禁止 null。"#;
...
规则：只能引用输入中的 ID；允许创建/更新密码组、创建空条目、从本地现有字段拆分条目、更新条目分类与结构；绝不输出字段 value；禁止删除条目、删除字段或覆盖字段值；字段拆分必须提供 source_entry_id、field_index、field_name、title；字段重命名必须同时提供 field_index、field_name、field_name_new；更新密码组至少提供新名称或简介；禁止 null。"#;
